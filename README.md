...
- **Hub**：创建节点、下发 Token、一键安装命令；SQLite 持久化；公开状态页
- **Agent**：采集 CPU / 内存 / Swap / 磁盘 / 上下行网速；探测三网延迟与丢包；HTTP JSON 心跳（默认 2s）
- **状态页**：深色卡片网格、在线/离线徽标、中文标签、移动端友好
...
打开 http://127.0.0.1:8080 ，右上角「管理」输入 `ADMIN_TOKEN` 可编辑节点。

## Docker Compose

```bash
export ADMIN_TOKEN=请换成强密码
...
| `ADMIN_TOKEN` | `changeme` | 管理 API / 页面 Token |
...
| POST | `/api/admin/nodes` | 创建节点（头 `X-Admin-Token`） |
| GET/PATCH/DELETE | `/api/admin/nodes` / `{id}` | 管理节点 |
| GET | `/api/admin/nodes/{id}/install` | 安装命令 |
...
```
cmd/hub          Hub 入口（embed 静态页）
cmd/agent        Agent 入口
internal/hub     HTTP API + SQLite
...
Self-hosted Hub + Agent monitor for multiple VPS nodes. Hub stores node metadata (traffic left, bandwidth, renewal, price) in SQLite; agents report CPU/mem/swap/disk/net rates and China ISP latency/loss every ~2s. Public dark status page; admin via `ADMIN_TOKEN`. One-line Linux install and Windows PowerShell helper. No plugins, web terminal, or remote exec.

```bash
ADMIN_TOKEN=changeme go run ./cmd/hub
go run ./cmd/agent --hub http://127.0.0.1:8080 --token <token>
